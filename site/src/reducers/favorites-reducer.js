import {
	LOAD_FAVORITES_STARTED,
	LOAD_FAVORITES_SUCCESS,
	LOAD_FAVORITES_FAILED,
	CHANGE_FAVORITES_GROUPS,
	SET_EDITABLE,
	FAVORITE_GROUP_ADDED,
	FAVORITE_GROUP_DELETED,
	MOVE_GROUP,
...
import {
	PICTURE_FROM_FAVOTIRES_DELETED,
...
export default (state = initState, action) => {
	switch (action.type) {
		...
		case MOVE_GROUP: {
			...
			if (
				(moveGroupIndex === 0 && direction === DIRECTION_UP) ||
			...
			const toIndex =
				moveGroupIndex + (direction === DIRECTION_UP ? -1 : 1);
		...
		case PICTURE_FROM_FAVOTIRES_DELETED: {
			...
			const newGroups = groups.filter(() => true).concat([otherGroup]);
			...
			const itemIndex = groupWithItem.items.findIndex(
				(i) => i.pictureId === id
			);
		...
		case DELETE_PICTURE_SUCCESS: {
			...
			newOtherGroup.items = otherGroup.items.filter(
				(i) => i.pictureId !== id
			);
