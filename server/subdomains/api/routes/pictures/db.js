		...
		checkId(museumId)
			...
			.then(() => {
				...
				return updateId === "current"
					...
					: getOldPicturesByMuseumIdAndReleaseId(
							searchText,
							sortedField,
							sortedType,
							mid,
			...
			.then((ps) => (pictures = ps))
			.then(() => {
				return getPicturesPagesDataByRequest(
					id,
					searchText,
					limit,
					pageNumber
			...
			.then((pagesData) => {
				return resolve({ pictures, pagesData, museumId: mid });
			})
			.catch(reject);
	});
...
exports.getPictureById = (id) =>
	new Promise((resolve, reject) => {
		db.get(
			`SELECT p.id, p.museum_id, p.name, p.description, p.qrcode, p.include_release includeRelease,
                (
...
exports.addPicture = (museumId, name, description, qrcode) =>
	new Promise((resolve, reject) => {
	...
	new Promise(async (resolve, reject) => {
		db.get(
			...
			(err, { pagesCount }) => {
				...
				console.log(searchText, pagesCount, limit);
				resolve({
...
const getPictures = (
	...
	new Promise((resolve, reject) => {
		...
		const sortedQuery =
			sortedField === "none"
				? ""
				: `ORDER BY p.${sortedFieldsTransform(
						sortedField
				  )} ${sortedType}`;
		db.all(
			`SELECT p.id, p.name, p.description, p.qrcode, p.include_release includeRelease,
            EXISTS (
                SELECT f.id 
                FROM favorites_items f 
                WHERE f.picture_id=p.id
            ...
            ${sortedQuery}
            LIMIT ?, ? `,
			[museumId, `%${searchText}%`, (pageNumber - 1) * limit, limit],
			(err, pictures) => {
				if (err) return reject(serverError());
				resolve(pictures);
			}
...
exports.getPicturesByMuseumId = (museumId) =>
	new Promise((resolve, reject) => {
		db.all(
			`SELECT id, museum_id museumId, qrcode, include_release includeRelease
            FROM pictures 
            WHERE museum_id=?`,
			[museumId],
			(err, pictures) => {
...
exports.getReleasedPicturesByMuseumId = (museumId) =>
	new Promise((resolve, reject) => {
		db.all(
			`SELECT id, museum_id museumId, name, description, qrcode
...
const addPicture = (museumId, name, description, qrcode) =>
	new Promise((resolve, reject) => {
		db.run(
			`INSERT INTO pictures (museum_id, name, description, qrcode, include_release)
...
const getPictureByAll = (museumId, name, description, qrcode) =>
	new Promise((resolve, reject) => {
		db.get(
			`SELECT p.id, p.name, p.description, p.qrcode, p.include_release includeRelease,
                (
            ...
            LIMIT 1`,
			[museumId, name, description, qrcode],
			(err, picture) => {
				if (err) return reject(serverError());
				if (!picture) return reject(serverError());
				resolve(picture);
...
const getOldPicturesByMuseumIdAndReleaseId = (
	searchText,
	sortedField,
	sortedType,
	museumId,
	...
	new Promise((resolve, reject) => {
		...
		db.all(
			`SELECT p.id, p.museum_id, p.name, p.description, p.qrcode, p.release_id releaseId,
