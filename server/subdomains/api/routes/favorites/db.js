...
exports.addFavoritePicture = (pictureId) =>
	new Promise((resolve, reject) => {
		db.run(
			`INSERT INTO favorites_items (group_id, picture_id, _index)
            VALUES (-1, ?, 0)`,
			[pictureId],
			(run, err) => {
...
exports.deleteFavoritePictureByPictureId = (id) =>
	new Promise((resolve, reject) => {
		db.run(
			`DELETE FROM favorites_items WHERE picture_id=?`,
			[id],
			(run, err) => {
				if (err || run) return reject(serverError());
				resolve();
	...
	return new Promise((resolve, reject) => {
		...
		groups.forEach((group, index) => {
			...
			if (id !== -1) {
				...
				db.run(
					`UPDATE favorites_groups
                        SET name=?, _index=?, description=?
                        WHERE id=?`,
					[name, index, description, id],
					(run, err) => {
						if (err) return reject({ error: SERVER_ERROR });
						group.items.forEach((item, index) => {
							db.run(
								`UPDATE favorites_items
                                SET group_id=?, _index=?
                                WHERE id=?`,
...
const getItemsInGroups = (groups) =>
	new Promise((resolve, reject) => {
		let allItems = [];
		let _index = 0;
		groups.forEach((g, index) => {
			db.all(
				`SELECT id, picture_id pictureId
...
const getPictureForItems = ({ items, groups }) =>
	new Promise((resolve, reject) => {
		let _index = 0;
		if (items.length === 0) return resolve({ groups });
		items.forEach((i) => {
			require("../pictures/db")
				.getPictureById(i.pictureId)
...
const addFavoriteGroup = (userId, name, description) =>
	new Promise((resolve, reject) => {
		db.run(
			`INSERT INTO favorites_groups (user_id, name, description)
            VALUES (?, ?, ?)`,
...
const getLastFavoriteGroupByData = (name, description) =>
	new Promise((resolve, reject) => {
		db.get(
			`SELECT id, name, description 
            FROM favorites_groups
...
const clearFavoriteItemsByGroupId = (id) =>
	new Promise((resolve, reject) => {
		db.run(
			`UPDATE favorites_items
            SET group_id=?
