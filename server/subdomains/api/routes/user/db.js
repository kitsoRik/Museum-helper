...
exports.checkVerifyLink = (link) =>
	new Promise((resolve, reject) => {
		db.get(
			`SELECT email, link FROM email_verify_links WHERE link=?`,
			[link],
			(err, row) => {
...
exports.getVerifyLink = (email) =>
	new Promise((resolve, reject) => {
		db.get(
			`SELECT link FROM email_verify_links WHERE email=?`,
			[email],
			(err, row) => {
				if (err) return reject(serverError());
				resolve(row.link);
...
exports.checkVerifyEmail = (email) =>
	new Promise((resolve, reject) => {
		db.get(
			`SELECT email, link FROM email_verify_links WHERE email=?`,
			[email],
			(err, row) => {
				if (err) return reject(serverError());
				resolve(row);
...
exports.removeVerifyEmail = (link) =>
	new Promise((resolve, reject) => {
		db.run(
			`DELETE FROM email_verify_links WHERE link=?`,
			[link],
			(run, err) => {
...
exports.changeUserData = (id, changes) =>
	new Promise((resolve, reject) => {
		let key = Object.keys(changes)[0];
		let f;
		if (key === "password")
			f = this.changePassword(id, hashPassword(changes[key]));
		console.log(key);
		f.then((d) => resolve({ changes: d })).catch(reject);
	});

exports.loginIn = (email, password) =>
	new Promise((resolve, reject) => {
		let user;
